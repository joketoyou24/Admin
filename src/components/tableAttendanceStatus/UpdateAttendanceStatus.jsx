import "./updateAttendanceStatus.scss";
import UpdateIcon from "@mui/icons-material/Update";
import ErrorMessage from "../LoginForm/ErrorMessage";

import React, { useEffect, useState } from "react";

const UpdateAttendanceStatus = ({ active, handleModal, token, id }) => {
  ...
  const handleUpdate = async () => {
    ...
    const response = await fetch(`/attendance_status/${id}`, requestOptions);
    if (!response.ok) {
      setErrorMessage("Something went wrong when updating attendance status");
    } else {
      cleanFormData();
      handleModal();
    }
